...
public class RegexUtil {
	 ...
	 * @return String
	 */
	private String removeMinBrackets(StringBuffer str){
		return str.toString().replaceAll("^\\(", "").replaceAll("\\)$", "");
	}
	 ...
	 * @return boolean 是 true，否则 false。
	 */
	private boolean chkMinBracket(StringBuffer sb){
		if("(".equals(sb.substring(0, 1)) && ")".equals(sb.substring(sb.length()-1))){
			return true;
		}else{
			return false;
	...
	public void or(RegexUtil re){
		...
		if(chkMinBracket(sb)){
			//首先去掉两边的中括号
			sb=new StringBuffer(removeMinBrackets(sb));
		}
		sb.append("|"+re.toString());
		//追加上小括号
		sb=new StringBuffer(addMinBrackets(sb));
	}
