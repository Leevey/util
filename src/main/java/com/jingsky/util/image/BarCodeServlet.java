 ...
 * &lt;servlet-class>com.jingsky.util.image.BarCodeServlet&lt;/servlet-class><br/>
 * &lt;/servlet> ...<br/>
 * &lt;servlet-mapping><br/>
 * &lt;servlet-name>CreateBarCode&lt;/servlet-name><br/>
 ...
 * CODE39,CODE39EXT,INTERLEAVED25,CODE11,CODABAR
 * ,MSI,UPCA,IND25,MAT25,CODE93,EAN13
 * ,EAN8,UPCE,CODE128,CODE93EXT,POSTNET,PLANET,UCC128 .<br/>
 * code: 要打印的条码内容. <br/>
 ...
 * name>|<style>|<size>.Style可以是PLAIN,ITALIC或BOLD. <br/>
 * fontColor(默认为黑色):
 * 条码文本的颜色,有效值为RED,BLUE,GREEN,BLACK,GRAY,LIGHTGRAY,WHITE,DARKGRAY
 * ,YELLOW,ORANGE,CYAN和MAGENTA. <br/>
 ...
 * ------------------------------------<br/>
 * 其中关键的参数主要是 barType,code
 * 这2个.另外把checkCharacter和checkCharacterInText这2个参数设置成n,如果是y的话
 * ,会在你的条形码后面追加一个校验码,虽然不影响使用(扫描设备扫描条形码得到数据不会出现校验码),但是看着比较别扭.<br/>
 * <br/>
 * 5.还可以在applet中使用,具体使用参数请对照net.maxt.test.BarCodeApplet.java文件,与WEB使用方式差不多. <br/>
 * 
 * @author 朱志杰 QQ：695520848 Jul 16, 2013 9:34:55 AM
 */
public class BarCodeServlet extends HttpServlet {
	...
	private BarCode getChart(HttpServletRequest httpservletrequest) {
		...
		try {
			...
			if (httpservletrequest.getParameter("width") != null
				...
				setParameter("height",
						httpservletrequest.getParameter("height"));
			...
			setParameter("textFont",
					httpservletrequest.getParameter("textFont"));
			setParameter("fontColor",
					httpservletrequest.getParameter("fontColor"));
			setParameter("barColor",
					httpservletrequest.getParameter("barColor"));
			setParameter("backColor",
					httpservletrequest.getParameter("backColor"));
			setParameter("rotate", httpservletrequest.getParameter("rotate"));
			setParameter("barHeightCM",
					httpservletrequest.getParameter("barHeightCM"));
			setParameter("x", httpservletrequest.getParameter("x"));
			setParameter("n", httpservletrequest.getParameter("n"));
			setParameter("leftMarginCM",
					httpservletrequest.getParameter("leftMarginCM"));
			setParameter("topMarginCM",
					httpservletrequest.getParameter("topMarginCM"));
			setParameter("checkCharacter",
					httpservletrequest.getParameter("checkCharacter"));
			setParameter("checkCharacterInText",
					httpservletrequest.getParameter("checkCharacterInText"));
			setParameter("Code128Set",
					httpservletrequest.getParameter("Code128Set"));
			setParameter("UPCESytem",
					httpservletrequest.getParameter("UPCESytem"));
	...
	/**
	 * 产生条形码,详细参数见类注释。临时新增二维码生成，原条形码生成不受影响
	 */
	public void doGet(HttpServletRequest httpservletrequest,
		...
		if (StringUtil.isEmpty(type)) {
			try {
				...
				if (barcode1.autoSize) {
					BufferedImage bufferedimage = new BufferedImage(
							barcode1.getSize().width,
							barcode1.getSize().height, 13);
					java.awt.Graphics2D graphics2d = bufferedimage
							.createGraphics();
				...
				java.awt.Graphics2D graphics2d1 = bufferedimage1
						.createGraphics();
			...
			//二维码
			String content = URLDecoder.decode(httpservletrequest.getParameter("content"),"utf-8");
			String width = httpservletrequest.getParameter("width");
			String height = httpservletrequest.getParameter("height");
			String isShowContent = httpservletrequest.getParameter("isShowContent");
			ZXingUtil.encodeQRCodeImage(content, null, "jpg", Integer.parseInt(width), Integer.parseInt(height), null, httpservletresponse.getOutputStream()
					,StringUtil.isEmpty(isShowContent) ? false : ("true".equals(isShowContent) ? true : false));
		}
	}
