    ...
    implements Serializable
{
	 ...
	 * 生成条形码
	 */
	public void makeBarCode(int width,int height,String text,String imgPath){
		BarCode barcode1 = getChart(width,height,text);
		...
		// 生成二维码QRCode图片
		try {
			ImageIO.write(bufferedimage1, "jpg", imgFile);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @return BarCode
	 */
	private BarCode getChart(int width,int height,String text) {
		if (barcode == null)
			barcode = new BarCode();
		try {
	...
	private void setParameter(String s, String s1) {
		if (s1 != null)
			if (s.equals("code"))
				barcode.code = s1;
